Facter.add(:ipa_replicascheme, :timeout => 10) do
  setcode do
    ...
    servers = Facter::Util::Resolution.exec("/sbin/runuser -l admin -c 'kinit -kt ~admin/admin.keytab admin && /usr/sbin/ipa-replica-manage list' 2>/dev/null | /bin/egrep -v '#{fqdn}|winsync' | /bin/cut -d: -f1")
